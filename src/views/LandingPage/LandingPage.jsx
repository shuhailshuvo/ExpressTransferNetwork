...
class LandingPage extends React.Component {
  ...
  startLocation = location => {
    this.setState({ startLocation: location, selectLocation: false, action:"LOCATION", error:false });
  };
  ...
  endDate = date => {
    this.setState({ endDate: date, endDateString: date.format("D-M-Y"),action:"DATE",error:false });
  };
  passengers = number => {
  ...
  bags = number => {
    this.setState({ bags: number,  withBags: number ? true : false,error:false });
  };

  getQuotes = (action) => {
    let date =moment();
    const { startLocation, endLocation, startDateString,endDateString, passengers, travellingBothWay, withBags, bags } = this.state;
    switch(action){
      case "LOCATION":
        if (startLocation && endLocation) {
          this.setState({ selectLocation: true, action:"DATE", error:false, startDateString: date.format("D-M-Y") });
        }else{
      ...
      case "DATE":
        if (startDateString && (!travellingBothWay||endDateString)) {
          this.setState({ selectDates: true, action:"PASSENGER", error:false });
        }else{
          this.setState({ error: true });
        }
        break;
      case "PASSENGER":
        if (passengers && (!withBags||bags)) {
          this.setState({ loading:true,selectPassengers: false, action:"SEARCH", error:false });
        }else{
          this.setState({ error: true });
        }
        break;
  ...
  travellingBothWay = () => {
    ...
    this.setState({ error:false,travellingBothWay: returnJourney, endDate: returnJourney?endDate:null, endDateString: returnJourney?endDate.format("D-M-Y"):"",action:"DATE" });
  };
  ...
  >
    ...
    {this.state.selectDates && (
        ...
        >
          ...
          <b>, Returns:
          <i>{this.state.endDateString}</i></b>
          )}
        </h4>
      )}
  </CardHeader>
